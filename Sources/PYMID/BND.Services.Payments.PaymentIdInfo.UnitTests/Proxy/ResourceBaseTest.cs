using BND.Services.Infrastructure.ErrorHandling;
using NUnit.Framework;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Text;
using System.Threading.Tasks;

namespace BND.Services.Payments.PaymentIdInfo.Proxy.Test
{
    [TestFixture]
    public class ResourceBaseTest
    {
        private string _token = "token";
        private string _baseAddress = "http://test.org";

        private Model1 _model = new Model1
        {
            Id = 20,
            Names = new string[] { "Test1", "Test2" },
            Scores = new List<int> { 12, 548, 89 },
            Model = new Model2 { Id = 44, Name = "JJJJ" },
            Models = new Model3[]
            {
                new Model3 { Code = 22, BirthDate = DateTime.Now.AddYears(-10) },
                new Model3 { Code = 33, BirthDate = DateTime.Now.AddYears(-20) }
            },
            Structs = new List<Struct1>
            {
                new Struct1 { Key = "Key01", Value = "xxxxx" },
                new Struct1 { Key = "Key02", Value = 20d }
            },
            Dict = new Dictionary<string, Model4>
            {
                { "Dict1", new Model4 { Val = 45.856, Int = 456 } },
                { "Dict2", new Model4 { Val = 45.856, Int = 456 } },
                { "Dict3", new Model4 { Val = 45.856, Int = 456 } }
            },
            Models5 = new Model5[]
            {
                new Model5 {Id = 1, Url = new Uri("http://localhost/")},
                new Model5 {Id = 1, Url = null}
            },
            Test = null
        };
        [Test]
        public void MapModelToKeyValuePairs_Should_WorkFind_When_EverythingFind()
        {
            MockClass cl = new MockClass(_baseAddress, _token);
            cl.MapModelTest(_model);
        }

        [Test]
        public void SetResult_Should_ThrowError_When_ResultNotJson()
        {
            HttpResponseMessage obj = new HttpResponseMessage();
            obj.StatusCode = System.Net.HttpStatusCode.InternalServerError;
            obj.Content = new StringContent("error");

            MockClass cl = new MockClass(_baseAddress, _token);
            Assert.Throws(typeof(ProxyException), () => cl.SetResult<string>(obj));
        }

        public class Model1
        {
            public int Id { get; set; }
            public string[] Names { get; set; }
            public List<int> Scores { get; set; }
            public Model2 Model { get; set; }
            public Model3[] Models { get; set; }
            public Model5[] Models5 { get; set; }
            public List<Struct1> Structs { get; set; }
            public Dictionary<string, Model4> Dict { get; set; }
            public string Test { get; set; }
        }

        public class Model2
        {
            public int Id { get; set; }
            public string Name { get; set; }
        }

        public class Model3
        {
            public int Code { get; set; }
            public DateTime BirthDate { get; set; }
        }

        public struct Struct1
        {
            public string Key { get; set; }
            public object Value { get; set; }
        }

        public class Model4
        {
            public double Val { get; set; }
            public int Int { get; set; }
        }

        public class Model5
        {
            public int Id { get; set; }
            public Uri Url { get; set; }
        }
    }

}

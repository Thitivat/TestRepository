...
namespace BND.Services.Payments.PaymentIdInfo.Proxy.Test
{
    ...
    public class PaymentIdInfoProxyTest
    {
        ...
        public void GetFilterTypes_Should_Return_FilterTypes()
        {
            ...
            expected.Add(EnumFilterType.Unknown);
            expected.Add(EnumFilterType.iDeal);
            expected.Add(EnumFilterType.eMandates);
            expected.Add(EnumFilterType.CreditTransfer);
        ...
        {
            // Set expected data that will return from mock http client.
            _httpMessageHandler.AddMockResponse("PaymentIdInfos/BndIban/" + _bndIban + "?filtertype=Unknown", HttpStatusCode.OK, _expectedResponse);

            using(IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                var actual = paymentIdInfo.GetByBndIban(_bndIban, new List<EnumFilterType>() { EnumFilterType.Unknown });
        ...
        public void GetByBndIban_WithFilterType_Should_Return_PaymentIdInfoModel()
        {
            ...
            using (IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                ...
                Assert.AreEqual(_expectedResponse.First().BndIban, actual.First().BndIban);
                Assert.True(actual.All(c => c.BndIban == _expectedResponse.First().BndIban));
                Assert.True(actual.All(c => c.TransactionType == _filterType.First().ToString()));
            }
        }
        ...
        public void GetByBndIban_Should_ThrowError_When_NotSendBndIban()
        {
            ...
            using (IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                Assert.Throws(typeof(ProxyException), () => paymentIdInfo.GetByBndIban(String.Empty, new List<EnumFilterType>() { EnumFilterType.Unknown }));
            }
        }
        ...
        {
            // Set expected data that will return from mock http client.
            _httpMessageHandler.AddMockResponse("PaymentIdInfos/SourceIban/" + _sourceIban + "?filtertype=Unknown", HttpStatusCode.OK, _expectedResponse);

            using (IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                var actual = paymentIdInfo.GetBySourceIban(_sourceIban, new List<EnumFilterType>() { EnumFilterType.Unknown });
        ...
        public void GetBySourceIban_Should_ThrowError_When_NotSendSourceIban()
        {
            ...
            using (IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                Assert.Throws(typeof(ProxyException), () => paymentIdInfo.GetBySourceIban(String.Empty, new List<EnumFilterType>() { EnumFilterType.Unknown }));
            }
        }
        ...
        {
            // Set expected data that will return from mock http client.
            _httpMessageHandler.AddMockResponse("PaymentIdInfos/Transaction/" + _transactionId + "?filtertype=Unknown", HttpStatusCode.OK, _expectedResponse);

            using (IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                var actual = paymentIdInfo.GetByTransactionId(_transactionId, new List<EnumFilterType>() { EnumFilterType.Unknown });
        ...
        public void GetByTransactionId_Should_ThrowError_When_NotSendTransactionId()
        {
            ...
            using (IPaymentIdInfoProxy paymentIdInfo = new PaymentIdInfoProxy(_baseAddress, _token, _mockHttpClient))
            {
                Assert.Throws(typeof(ProxyException), () => paymentIdInfo.GetByTransactionId(String.Empty, new List<EnumFilterType>() { EnumFilterType.Unknown }));
            }
        }
    }
}

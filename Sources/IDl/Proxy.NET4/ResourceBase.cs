    ...
    public abstract class ResourceBase : IDisposable
    {
            ...
            where TResult : class
        {
            ...
            else
            {
                ...
                switch (responseMessage.StatusCode)
                {
                    ...
                    default:
                        throw new iDealOperationException(ErrorCode, Message);
                }
            }
        }
            ...
            where TModel : class
        {
            ...
            foreach (PropertyInfo prop in model.GetType().GetProperties())
            {
                ...
                else if (prop.PropertyType == typeof(Decimal)) // fix localization.
                {
                    // Adds data to result variable.
                    result.Add(
                        new KeyValuePair<string, string>(JoinStringOmitEmpty(Properties.Resources.SEPARATOR, parentPropertyName, prop.Name),
                                                          Convert.ToDecimal(prop.GetValue(model, null)).ToString(CultureInfo.CreateSpecificCulture("en-US")))
                    );
                }
                // For value.
                else

...
namespace BND.Services.Payments.eMandates.Proxy
{
            ...
            where TResult : class
        {
            ...
            else
            {
                ...
                switch (responseMessage.StatusCode)
                {
                    ...
                    default:
                        throw new eMandateOperationException(ErrorCode, Message);
                }
            }
        }
            ...
            where TModel : class
        {
            ...
            foreach (PropertyInfo prop in model.GetType().GetProperties())
            {
                ...
                if (prop.PropertyType.IsArray && IsClassOrStruct(prop.PropertyType.GetElementType()) ||
                    IsGenericCollection(prop.PropertyType) && IsClassOrStruct(prop.PropertyType.GetGenericArguments().First()) && prop.PropertyType.GetGenericArguments().FirstOrDefault() != typeof(TimeSpan))
                {
                ...
                else
                {
                    ...
                    if ((prop.PropertyType.IsArray || IsGenericCollection(prop.PropertyType)) && prop.PropertyType.GetGenericArguments().FirstOrDefault() != typeof(TimeSpan) && prop.PropertyType.GetGenericArguments().FirstOrDefault() != typeof(Decimal))
                    {
                        // Sets value to list object.
                    ...
                    else if (IsGenericCollection(prop.PropertyType) && prop.PropertyType.GetGenericArguments().FirstOrDefault() == typeof(TimeSpan))
                    {
                        result.Add(
                            new KeyValuePair<string, string>(JoinStringOmitEmpty(Properties.Resources.SEPARATOR, parentPropertyName, prop.Name),
                                                             (prop.GetValue(model) == null) ? null : XmlConvert.ToString((TimeSpan)prop.GetValue(model)))
                        );
                    }
                    // For static value.
                    else

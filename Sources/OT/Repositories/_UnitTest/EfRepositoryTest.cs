...
namespace BND.Services.Security.OTP.RepositoriesTest
{
    ...
    public class EfRepositoryTest
    {
        ...
        {
            //var actual = new MSTest.PrivateObject(new EfRepository<Poco1>(_dbContext));

            //Assert.IsNotNull(actual.GetField("Context"));
            //Assert.IsNotNull(actual.GetField("DbSet"));
        }
        ...
        {
            var actual = new EfRepository<Poco1>(_dbContext);
            Assert.Throws<ArgumentException>(() =>  actual.Get(page: new Page<Poco1>()));
        }
        ...
        {
            var actual = new EfRepository<Poco1>(_dbContext);
            Assert.Throws<InvalidOperationException>(() => actual.Get(page: new Page<Poco1> { OrderBy = p1 => p1.OrderBy(p => p.Id) }));
        }
        ...
        public void Test_Insert_Success_ByMultipleEntities()
        {
            ...
            Assert.AreEqual(_poco1sMockData.Count + expected.Count, actual.Get().Count());
        }

...
/* Initilaize data */
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumGenders])
BEGIN
   /* REGION [ EnumGenders ]*/
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumContactInfoTypes])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumSubjectTypes])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumStatuses])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumListTypes])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumIdentificationTypes])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumCountries])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumLanguages])
...
GO
IF NOT EXISTS(SELECT 1 FROM [sl].[EnumActionTypes])
BEGIN /* REGION [ EnumActionTypes ]*/
	DBCC CHECKIDENT ('[sl].[EnumActionTypes]', RESEED, 1)
	INSERT INTO  [sl].EnumActionTypes(Name, Description) VALUES
	('SanctionLists', 'All activities occurred about sanction list.')
END

GO
IF NOT EXISTS(SELECT 1 FROM [sl].[Settings])
BEGIN /* REGION [ Settings ]*/
	DBCC CHECKIDENT ('[sl].[Settings]', RESEED, 1)
	INSERT INTO  [sl].Settings(ListTypeId, [Key], Value) VALUES
	(1, 'RssUrl', 'http://ec.europa.eu/external_relations/cfsp/sanctions/list/version4/rss/rss_file.xml'),
	(2, 'RssUrl', 'https://zoek.officielebekendmakingen.nl/zoeken/resultaat/rss/?zkt=Uitgebreid&pst=Staatscourant&vrt=Terrorisme&zkd=AlleenInDeTitel&dpr=Alle&spd=20150816&epd=20150816&sdt=DatumPublicatie&rbk=Besluiten+van+algemene+strekking&owp=Openbare+orde+en+veiligheid+%7c+Terrorisme&org=Ministerie+van+Buitenlandse+Zaken+(MINBUZA)&orgt=ministerie&planId=&pnr=1&rpp=10'),
	(1, 'pubDateFormat','dd/MM/yyyy hh:mm:ss K'),
	(2, 'pubDateFormat', 'ddd, dd MMM yyyy HH:mm:ss K'),
	(1, 'AutoUpdate', '1'),
	(2, 'ManualUpdate', '1'),
	(3, 'ManualUpdate', '1'),
	(1, 'GlobalDateTimeFormat', 'yyyy-MM-dd')
END
GO